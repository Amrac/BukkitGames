...
public class BGChat {
	...
	public static void printKitChat(Player player) {
		if (plugin.ADV_CHAT_SYSTEM) {
			...
			player.sendMessage("");
			player.sendMessage("�bAvailable asdasdkits: �7(/kit [KitName])");
			player.sendMessage("");
			player.sendMessage("�aYour kisdasdts: �f" + yourkits);
			player.sendMessage("�aOther kasdasdits: �f" + otherkits);
			player.sendMessage("�bMore kiasdasdts available at: "
					+ plugin.KIT_BUY_WEB);
			player.sendMessage("");
		}
	}
